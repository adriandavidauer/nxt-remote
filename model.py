...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='keras model prediction from teachable machine')
    ...
    parser.add_argument('--cam', help='if this option is set the webcam will be used - therefore image path will be ignored', action='store_true')
    parser.add_argument('--cam_port', help='if this option is set the webcam will be used - therefore image path will be ignored')
    ...
    if not args.cam:
        ...
        while result:
            print(f"Prediction is {model.predict(image_path=None, image=image)}")
            result, image = cam.read()
